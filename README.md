...
- **Modo de lectura enfocada** (oculta índice, breadcrumb y chrome secundario), **modo "resumen de una página"** (un botón en el header reemplaza todo el acordeón por una síntesis de una sola pantalla armada con contenido ya existente — el congreso en un párrafo, el diagnóstico causal, las 3 recomendaciones principales y "Mi lectura" — con salida directa de vuelta al documento completo) y **botón "volver arriba"**.
...
### Alcance de la mejora en 5 frentes

La Fase 1 (alta prioridad) cubrió navegación, buscador, accesibilidad, trazabilidad de fuentes y exportación. La Fase 3 (baja prioridad) — mapa de constelación, modo resumen de una página y autodiagnóstico para IPS/aseguradores — ya está implementada; las tres piezas se derivan de datos y contenido que ya existían en la app (sin rúbricas ni comparadores inventados). Queda pendiente la Fase 2 (media prioridad): reorganización del contenido por conceptos con matriz de relaciones, comparadores adicionales, más plantillas descargables y un buscador de cifras por rango. No se construyeron comparadores ni calculadoras que habrían requerido inventar datos o metodologías que el congreso no proporcionó — incluida la calculadora de brecha SOAT/CUPS de la primera ronda de esta fase, retirada después por no aportar valor más allá de una multiplicación porcentual (ver Metodología y fuentes en la app).
